...
export default function Apply_loan() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCancelcount(
          res.data?.result.filter(
            (loan) => loan.status === "cancel" && loan.custid === id
          ).length
        );

        setApprovedcount(
          res.data?.result.filter(
            (loan) => loan.currentstatus === "Approved" && loan.custid === id
          ).length
        );
      } catch (err) {
        console.log(err);
  ...
  console.log("appliedcount: ", { appliedcount, cancelcount });
  let timestamp = Date();
  const formik = useFormik({
    initialValues: {
      ...
      loanId: window.crypto.randomUUID(),
    },
    onSubmit: (values) => {
      confirmAlert({
        customUI: ({ onClose }) => {
      ...
      const exe = () => {
        if (approvedcount == 1) {
          toast.error("Your request is already approved!", {
        ...
        } else if (appliedcount < 3 && cancelcount < 3) {
          const config = {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          ...
          axios
            .post(
              `${process.env.REACT_APP_API_PORT}/api/loan/add`,
              values,
              config
            )
    ...
    validationSchema: yup.object().shape({
      propertyrate: yup
        .string()
        .required("Property rate cannot be blank")
        .matches(/^\d+$/, "Invalid property rate"),
      loanamt: yup
        .number()
        .required("Loan Amount cannot be blank")
        .max(10000000, "Loan amount cannot exceed 10m"),
      loantenure: yup
        .number()
        .required("Loan tennure cannot be blank")
        .max(25, "Loan tenure cannot exceed 25 Years")
        .min(5, "Loan tenure cannot be less than 5 Years"),
      income: yup
        .string()
        .required("Income cannot be blank")
        .matches(/^\d+$/, "Invalid income"),
      occupation: yup
        .string()
            ...
            <form onSubmit={formik.handleSubmit}>
              <Stack spacing={2}>
                ...
                <TextField
                  ...
                  onBlur={formik.handleBlur}
                  error={
                    formik.touched.loanamt && Boolean(formik.errors.loanamt)
                  }
